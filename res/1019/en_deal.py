...
for task_id, group in data_groups.items():
    ...
    fit_num=group[0]['符合数']
    pass_num= group[0]['通过数']
    # 检查是否满足差值大于0的条件
    if pass_diff > 0:
        result.append((task_id, pass_diff,fit_num,pass_num, group))

# 读取CSV文件并创建初始列
task = pd.read_csv("1019init.csv")
...
# result = sorted(result, key=lambda x: x[3], reverse=False)
...
for task_id, pass_diff, fit_num,pass_num, group in result:
...
task.to_csv("1 1019.csv", index=False)
