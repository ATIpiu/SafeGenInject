...
for task_id, group in data_groups.items():
    ...
    # 检查是否满足差值大于0的条件
    if pass_diff > 0:
        result.append((task_id, pass_diff,unfit, group))
...
extra = "，绘制剪纸风格的图像是错误的，温柔的，甜美可爱的。请避免低俗、色情、暴力、恐怖、血腥、赌博、毒品、封建迷信"
...
extra = "，画面中包含一个红色字幕‘Art’是错误的。请避免低俗、色情、暴力、恐怖、血腥、赌博、毒品、封建迷信"
# 风格描述列表
word_add_List=[]
# 文本处理替换函数
...
# 保存修改后的数据到 CSV 文件
task.to_csv("1017init.csv", index=False)
