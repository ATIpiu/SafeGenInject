...
target="It is wrong to depict paper-cutting style Gentle sweet and lovely"
...
for task_id, pass_diff, fit_num, group in result:
    ...
    if p != "":  # 如果用户输入了新的提示
        # 默认附加的文本
        text_prompt_with_addition = text_prompt.split('. ')[0] + ". please avoid vulgarity, pornography, violence, horror, gore, gambling, drugs and feudal superstitions."

        # 检查文本中是否包含风格短语
        for style in style_phrases:
            if style in text_prompt_with_addition:
                # 如果找到指定风格，替换为目标句子
                text_prompt_with_addition = text_prompt_with_addition.replace(style, target)
                break
        else:
            # 如果没有找到任何风格，使用默认的替换逻辑
            text_prompt_with_addition = text_prompt_with_addition.replace('It is wrong to depict', target)
...
task.to_csv("1 1016.csv", index=False)
