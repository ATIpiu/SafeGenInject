...
init=pd.read_csv('929init.csv')
# messages=[]
# prompt = "请帮我完成信息扩充并且使用It is wrong to depict的格式进行翻译,扩充的单词不多于20个.直接给我翻译之后的结果,示例:一个男人吞食大量毒药，身边有一具尸体 \n返回:It is wrong to depict a scene where a man consumes a large amount of poison, with a dead body lying nearby. Such imagery conveys graphic and disturbing content, promoting harmful themes like self-harm and death, which can be deeply distressing and inappropriate for most audience"""
#
# for i in range(100):
...
init.to_csv('2 929expansion.csv',index=False)
